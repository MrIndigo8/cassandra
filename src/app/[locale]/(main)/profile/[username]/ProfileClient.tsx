...
function StatBox({ label, value }: { label: string; value: string | number }) {
  return (
    <div className="bg-surface rounded-xl border border-border/30 p-4 text-center">
...
function EmptyEntries({ isOwnProfile, username }: { isOwnProfile: boolean; username: string }) {
...
export default function ProfileClient({
  ...
  return (
    <div className="max-w-[860px] mx-auto px-4 py-8">
      <div className="relative rounded-2xl border border-border/40 overflow-hidden bg-surface/40">
        <div className="h-32 bg-gradient-to-r from-primary/20 via-secondary/10 to-transparent rounded-t-2xl" />
        ...
        <div className="px-6 pb-6 -mt-12">
          <div className="relative inline-block">
            {profile.avatar_url ? (
              ...
              <div className="w-20 h-20 rounded-full border-4 border-background bg-primary/20 flex items-center justify-center text-2xl font-bold text-primary">
                {profile.username[0].toUpperCase()}
              </div>
            )}
            <div
              className={`absolute -bottom-1 -right-1 w-6 h-6 rounded-full flex items-center justify-center text-xs border-2 border-background ${
                profile.role === 'architect'
                  ...
                  : profile.role === 'admin'
                    ...
                    : profile.role === 'moderator'
                      ...
                      : profile.role === 'oracle'
                        ? 'bg-amber-400 text-white shadow-glow-sm'
          ...
          <div className="mt-3">
            <h1 className="text-xl font-display font-bold text-text-primary">{displayTitle}</h1>
            ...
            <div className="flex flex-wrap items-center gap-3 mt-2 text-sm text-text-secondary">
              ...
              {profile.streak_count > 0 ? (
                <span className="flex items-center gap-1">
                  <span aria-hidden>🔥</span> {profile.streak_count} {t('days')}
                </span>
              ) : null}
              <span className="flex items-center gap-1">
                <span aria-hidden>📅</span> {t('since', { date: formatMemberSince })}
              </span>
            </div>
            ...
            <div className="flex items-center gap-3 mt-3 flex-wrap">
              ...
              <span className="flex items-center gap-1 text-sm font-mono text-primary">
                <span aria-hidden>⚡</span> {profile.rating_score.toFixed(1)}
              </span>
            </div>
            ...
            {isOwnProfile ? (
              <div className="mt-3 flex flex-wrap gap-2 items-center">
                <button type="button" onClick={() => setEditing(true)} className="btn-ghost text-sm flex items-center gap-1">
                ...
                {canAccessAdmin ? (
                  <>
                    ...
                    <Link
                      href="/admin/settings"
                      className="inline-flex items-center gap-2 rounded-lg border border-border bg-surface px-3 py-1.5 text-xs font-medium text-text-secondary hover:border-primary/40"
                    >
                      {t('settings')}
                    </Link>
                  </>
      ...
      {nextRole && !['oracle', 'architect'].includes(profile.role) ? (
        <div className="mt-6 bg-surface rounded-xl border border-border/30 p-5">
          ...
          <button
            ...
            className={`px-4 py-2 rounded-full text-sm font-medium transition-colors ${
              tab === key
                ...
                : 'bg-surface-hover text-text-secondary hover:text-text-primary border border-border/50'
            }`}
          >
            {label}
          </button>
      ...
      {tab === 'entries' ? (
        <div className="space-y-4 mt-6">
          {entries.length === 0 ? (
            <EmptyEntries isOwnProfile={isOwnProfile} username={profile.username} />
          ) : (
            entries.map((entry) => (
              <EntryCard
                key={entry.id}
